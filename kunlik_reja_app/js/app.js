document.addEventListener('DOMContentLoaded', () => {
    ...
    // Preset Goals
    const presetGoals = [
        { id: 'english', icon: '🇬🇧', name: 'Ingliz tili', tasks: ['Grammatika (30 min)', 'Yangi so\'zlar (15 ta)', 'Speaking mashqi'] },
        { id: 'sport', icon: '🏋️‍♂️', name: 'Sport', tasks: ['Yugurish (2 km)', 'Press (50 ta)', 'Cho\'zilish'] },
        { id: 'read', icon: '📚', name: 'Kitobxonlik', tasks: ['10 bet o\'qish', 'Konspekt qilish'] },
        { id: 'mind', icon: '🧘‍♂️', name: 'Ruhiyat', tasks: ['10 min meditatsiya', 'Shukronalik yozish'] },
        { id: 'code', icon: '💻', name: 'Dasturlash', tasks: ['1 ta masala ishlash', 'Yangi kod o\'rganish'] }
    ];
    ...
    function renderStories() {
        ...
        presetGoals.forEach((goal, i) => {
            const storyDiv = document.createElement('div');
            storyDiv.className = 'story-item';
            storyDiv.style.animationDelay = `${i * 0.1}s`;

            storyDiv.innerHTML = `
                <div class="story-icon-ring">
                    <div class="story-icon">${goal.icon}</div>
                </div>
                <div class="story-label">${escapeHtml(goal.name)}</div>
            `;

            storyDiv.addEventListener('click', () => {
                if (confirm(`"${goal.name}" uchun tayyor rejani doimiy qilib qo'shmoqchimisiz?`)) {
                    let addedCount = 0;

                    goal.tasks.forEach(taskText => {
                        const newTaskId = Date.now().toString() + Math.random();
                        const uniqueTemplateId = Date.now().toString() + Math.random();
                    ...
                    if (window.confetti) {
                        confetti({
                            ...
                            spread: 60,
                            origin: { y: 0.8 },
                            colors: ['#f09433', '#e6683c', '#bc1888']
                        });
                    }
                    showToast(`${addedCount} ta reja qo'shildi! ✨`);
                }
            });
    ...
    window.toggleTask = function (id) {
        ...
        if (t && t.status === 'completed') {
            ...
            // Show floating "Barakalla!" text
            const itemEl = document.querySelector(`[data-id="${id}"] .checkbox`);
            if (itemEl) {
                const rect = itemEl.getBoundingClientRect();
                const floatText = document.createElement('div');
                floatText.className = 'good-job-anim';
                floatText.textContent = 'Barakalla! 🎉';

                // Position relative to document
                floatText.style.left = `${rect.left + rect.width / 2 + window.scrollX}px`;
            ...
            // showToast("Ajoyib! Bajarildi ✅"); // Handled by visual anim
    ...
    function updateHistoryChart() {
        ...
        // Render the full range of days based on filter even if empty
        let numDays = 7;
        if (filter === 'month') numDays = 30;
        if (filter === 'all') numDays = Math.max(7, dateData.length); // At least 7

        const chartLabels = [];
        const chartData = [];
        const msInDay = 24 * 60 * 60 * 1000;

        // Build data points from X days ago up to today
        for (let i = numDays - 1; i >= 0; i--) {
            const dDate = new Date(todayTimestamp - (i * msInDay));
            const dString = dDate.toISOString().split('T')[0];
            const displayLabel = `${String(dDate.getMonth() + 1).padStart(2, '0')}/${String(dDate.getDate()).padStart(2, '0')}`;

            chartLabels.push(displayLabel);

            // Check if we have real data for this exact day
            const foundData = dateData.find(d => d.dateStr === dString);
            if (foundData) {
                const p = foundData.total === 0 ? 0 : Math.round(((foundData.completed + (foundData.partial * 0.5)) / foundData.total) * 100);
                chartData.push(p);
            } else {
                // If it's a past day or today with absolutely 0 tasks interacted with, it's 0%
                chartData.push(0);
            }
        }

        // Aggregate statistics for display below chart
        let totalCompleted = 0;
        let totalWeighted = 0;
        let totalCreated = 0;

        // Use filteredData for stats, not chartData, as chartData might have 0s for empty days
        const relevantDataForStats = dateData.filter(d => {
            const dTimestamp = new Date(d.dateStr).getTime();
            if (filter === 'week') return dTimestamp >= (todayTimestamp - 7 * msInDay);
            if (filter === 'month') return dTimestamp >= (todayTimestamp - 30 * msInDay);
            return true; // 'all' filter
        });

        relevantDataForStats.forEach(d => {
        ...
        // Render Chart
        renderHistoryChart(chartLabels, chartData);
        renderHabitGrid();
    }
        ...
        historyChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    pointRadius: data.length > 15 ? 3 : 5,
                    pointHoverRadius: 7
                }]
            },
            options: {
